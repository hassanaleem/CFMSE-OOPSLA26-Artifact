...
We provide logs generated by rerunning the experiments on the same server configuration used for the paper. These logs can be found in the `preexisting-logs` directory within the `fig7-coverage`. To generate plots from these files, follow the steps below: 

```bash
    cd fig7-coverage
    cd preexisting-logs
    cd <BENCHMARK>
    python3 ${KLEE_BUILD_DIR}/../scripts/coverage_graph.py \
    ./coverage-cfmse.csv \
    ./coverage-nocfmse.csv
```

### Figure 10 and 11 Bug Detection and Killed States 
...
We provide the logs used for the paper. Each benchmark subdirectory contains a `logs-paper` directory with these logs. To generate plots directly from these log files, run the following commands below:

```bash
    cd fig10-11-bug-detection-and-killed-states
    cd <BENCHMARK>
    python3 ../read_cfm_output.py \ 
    logs-paper <benchmark> seconds
```
### Table 4 Overhead
