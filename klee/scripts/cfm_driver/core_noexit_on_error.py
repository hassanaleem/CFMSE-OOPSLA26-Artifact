...
def run_main_noexit_on_error(input_bitcode, config, run_in_dir, dry_run=False):
    ...
    # if klee_options conatins -exit-on-error option, remove it
    if "--exit-on-error" in klee_options:
        klee_options = klee_options.replace("-exit-on-error", "")
    ...
    debug_print(f"Running KLEE without CFM on {input_bitcode} inside {klee_nocfm_dir}", tag="main")
