...
void llvm::writeCFGToDotFile(const Function &F, std::string Prefix) {
...
static bool runAnalysisOnly(Function &F, DominatorTree &DT,
  ...
  INFO("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
       "+++++++++++++++\n");
  INFO("Function name : " + F.getName().str() + "\n");
  INFO("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
       "+++++++++++++++\n");
  ...
  for (auto BBIt : post_order(&F.getEntryBlock())) {
    ...
    if (Utils::isValidMergeLocation(BB, DT, PDT) &&
      ...
      INFO("------------------------------------------------------------"
           "-------------------\n");
      INFO("Valid merge location found at BB " + BB.getNameOrAsOperand() +
           "\n");
      RegionAnalyzer MA(&BB, CFGInfo, Latency);
      MA.computeRegionMatch();
      MA.printAnalysis(errs());
      INFO("This merge is : " +
           std::string(MA.hasAnyProfitableMatch() ? "" : "NOT") +
           " PROFITABLE!\n");
    }
  }
...
bool llvm::runCFMSE(BasicBlock *BB, DominatorTree &DT, PostDominatorTree &PDT,
                    TargetTransformInfo &TTI) {
  bool LocalChange = false;
  if (Utils::isValidMergeLocation(*BB, DT, PDT)) {
    Function *Func = BB->getParent();
    ControlFlowGraphInfo CFGInfo(*Func, DT, PDT, TTI);
    // analyze regions using SE cost model
    RegionAnalyzer RA(BB, CFGInfo, CostModelTy::SE);
    RA.computeRegionMatch();
    RA.computeDCICompatibility();

    // we don't care about the profitability of the match
    unsigned BestMatchIdx = RA.getMostProfitableRegionMatchIndex();

    if (RA.isRegionMatchCompatibleForDCI(BestMatchIdx)) {
      RegionMelder RM(RA, true);
      RM.merge(BestMatchIdx);
      LocalChange = true;
    }
  ...
  if (Utils::isValidMergeLocation(*BB, DT, PDT)) {
    ...
    if (RA.hasAnyProfitableMatch()) {
      ...
      for (unsigned I : IdxsToProcess) {
        ...
        RegionMelder RM(RA, InsertDeadCode);
        RM.merge(I);
...
static bool runImplCodeSize(Function &F, DominatorTree &DT,
                            PostDominatorTree &PDT, LoopInfo &LI,
                            TargetTransformInfo &TTI) {
  INFO("Procesing function : " + F.getName().str() + "\n");
  Function *Func = &F;
  ...
  do {
    ...
    for (BasicBlock *BB : post_order(&Func->getEntryBlock())) {
      if (Utils::isValidMergeLocation(*BB, DT, PDT)) {
      DEBUG("Valid merge location found at block " + BB->getNameOrAsOperand() +
             "\n");
        ControlFlowGraphInfo CFGInfo(*Func, DT, PDT, TTI);
        RegionAnalyzer RA(BB, CFGInfo);
        RA.computeRegionMatch();
        ...
        if (RA.hasAnyProfitableMatch()) {
          ...
          for (unsigned I = 0; I < ClonedRA.regionMatchSize(); ++I) {
            ...
            DEBUG("Size changed from " + std::to_string(SizeBefore) + " to " +
                      std::to_string(SizeAfter) + " : " +
                      std::to_string(SizeBefore - SizeAfter) + " : " +
                      std::to_string((SizeBefore > SizeAfter)
                                         ? "Profitable"
                                         : "Unprofitable") +
  ...
  if (Changed) {
    ...
    INFO("Size reduction for function " + F.getName() + ": " +
         std::to_string(OrigCodeSize) + " to  " +
         std::to_string(FinalCodeSize) + " (" +
         std::to_string(PercentReduction) + "%)\n");
  }
...
static bool runImpl(Function &F, DominatorTree &DT, PostDominatorTree &PDT,
  ...
  do {
    ...
    for (auto &BBIt : post_order(&F.getEntryBlock())) {
      ...
      // check if this BB is the enrty to a diamond shaped control-flow
      if (Utils::isValidMergeLocation(BB, DT, PDT)) {
        DEBUG("Valid merge location found at BB " << BB.getNameOrAsOperand()
                                                  << "\n");
        ...
        if (MA.hasAnyProfitableMatch()) {
          ...
          if (LocalChange) {
            if (!NoSimplifyCFGAfterMelding) {
              DEBUG("Running CFG simplification\n");
              if (simplifyFunction(
                      F, TTI,
                      SimplifyCFGOptionsObj.setSimplifyCondBranch(false)
