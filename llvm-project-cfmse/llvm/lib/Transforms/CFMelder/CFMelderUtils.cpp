...
bool Utils::isValidMergeLocation(BasicBlock &BB, DominatorTree &DT,
  ...
  if (!PDT.getNode(&BB)->getIDom()->getBlock()) {
    DEBUG("No IPDOM for block " + BB.getNameOrAsOperand() +
...
bool Utils::containsUnhandledInstructions(const BasicBlock *BB) {
  for (const Instruction &I : *BB) {
    if (I.getOpcode() == Instruction::Switch ||
      ...
      return true;
    // check if load instruction is vector load
    if (const LoadInst *LI = dyn_cast<LoadInst>(&I)) {
      if (LI->getType()->isVectorTy())
        return true;
    }
    // check if store instruction is vector store
...
Utils::computeLatReductionAtBest(BasicBlock *BB1, BasicBlock *BB2,
  ...
  for (auto It = BB1->begin(); It != BB1->end(); ++It) {
    ...
    FreqMap[It->getOpcode()].first += InstructionCostModel.getCost(&(*It));
    TotalLatency += InstructionCostModel.getCost(&(*It));
  }
  ...
  for (auto It = BB2->begin(); It != BB2->end(); ++It) {
    ...
    FreqMap[It->getOpcode()].second += InstructionCostModel.getCost(&(*It));
    TotalLatency += InstructionCostModel.getCost(&(*It));
  }
...
double Utils::computeBlockSimilarity(BasicBlock *BB1, BasicBlock *BB2,
  ...
  auto LatInfo = computeLatReductionAtBest(BB1, BB2, InstructionCostModel);
  ...
  for (auto *BB : Replicated->blocks()) {
    ...
    // else you have to pay one more branch to execute the block conditionally
      ...
      for (Instruction &I : *BB) {
        TotalLatency += InstructionCostModel.getCost(&I);
      }
      LatReductionAtBest -= InstructionCostModel.getBrCost();
    }
  }
...
double Utils::computeRegionSimilarity(
  ...
  for (auto It : Mapping) {
    ...
    if (It.first == LExit)
      continue;
    auto LatInfo =
        computeLatReductionAtBest(It.first, It.second, InstructionCostModel);
...
int Utils::getGPUInstructionCost(const Instruction *I) {
  int SavedCycles = 0;
  switch (I->getOpcode()) {
...
int Utils::getSEInstructionCost(const Instruction *I) {
  int Priority = 2;
  // switch (I->getOpcode()) {
  // case Instruction::Load:
  // case Instruction::Store:
