...
void RegionInstructionAlignment::linearizeBb(
    BasicBlock *BB, SmallVectorImpl<Value *> &LinearizedVals) {
...
AlignedSeq<Value *>
RegionInstructionAlignment::computeBlockAlignment(BasicBlock *LeftBb,
                                                  BasicBlock *RightBb) {
...
void RegionInstructionAlignment::compute(
  ...
  for (auto It = BbMap.begin(); It != BbMap.end(); It++) {
    ...
    Result.concat(computeBlockAlignment(LBB, RBB));
  }
}

void RegionInstructionAlignment::printStats(llvm::raw_ostream &OS) {
  int SavedCycles = 0;
  ...
  INFO("Number of cycles saved by alignment : " + std::to_string(SavedCycles) +
       "\n");
}

RegionAnalyzer::RegionAnalyzer(BasicBlock *BB, ControlFlowGraphInfo &CFGInfo,
                               CostModelTy CostTy)
    : CostModelToUse(CostTy), DivergentBB(BB), CFGInfo(CFGInfo) {
  DEBUG("Using " +
        std::string(CostModelToUse == CodeSize  ? "code size"
                    : CostModelToUse == Latency ? "latency"
                                                : "symbolic execution") +
        " cost model\n");
  ...
  // select the instruction cost function
  if (CostModelToUse == SE) {
    InstrCostModel = std::make_unique<SEInstructionCost>();
  } else if (CostModelToUse == Latency) {
    InstrCostModel = std::make_unique<GPUInstructionCost>();
  } else {
    InstrCostModel =
        std::make_unique<CodeSizeInstructionCost>(&CFGInfo.getTTI());
  }
}

void RegionAnalyzer::computeSARegionMatch() {
  RegionScoringFunc ScoringFunc(*InstrCostModel);
  auto SMSA =
  ...
  for (auto Entry : Result) {
    ...
    if (Entry.match()) {
      ...
      RC.getMapping(BlockMap);
      double SimilarityScore = Utils::computeRegionSimilarity(
          BlockMap, L->getExit(), *InstrCostModel);
      std::shared_ptr<MergeableRegionPair> RegionPair =
          std::make_shared<MergeableRegionPair>(*L, *R, RC, SimilarityScore);
  ...
  INFO("Number of aligned region pairs : " + std::to_string(AlignedReginPairs) +
       "\n");
}
  ...
  for (Region *LRegion : LeftRegions) {
    for (Region *RRegion : RightRegions) {
      ...
      if (RC.compare()) {
        ...
        RC.getMapping(BlockMap);
        double SimilarityScore = Utils::computeRegionSimilarity(
            BlockMap, LRegion->getExit(), *InstrCostModel);
        std::shared_ptr<MergeableRegionPair> RegionPair =
...
// check for instructions with side effects that can not be
// fixed by the DCI
// e.g : call, invoke,
// FIXME : alloca is not supported by DCI
static bool checkDCICompatible(const AlignedSeq<Value *> &AlignedSeq) {
  for (auto &Entry : AlignedSeq) {
    if (Entry.missMatch()) {
      const Value *Unaligend =
          Entry.getLeft() ? Entry.getLeft() : Entry.getRight();
      const Instruction *UnaligendI = dyn_cast<Instruction>(Unaligend);
      if (UnaligendI) {
        unsigned int Op = UnaligendI->getOpcode();
        if (Op == Instruction::Call || Op == Instruction::Invoke ||
            Op == Instruction::Alloca) {
          return false;
        }
      }
    }
...
void RegionAnalyzer::computeDCICompatibility() {
  // compute instruction alignment

  if (HasBbMatch) {
    RegionInstructionAlignment RIAlignment(getInstrCostModel());
    DenseMap<BasicBlock *, BasicBlock *> BlockMap;
    BlockMap.insert(BestBbMatch);
    RIAlignment.compute(BlockMap);
    auto InstrAlignment = RIAlignment.getResult();
    DCICompatibility.push_back(checkDCICompatible(InstrAlignment));
    return;
  }

  for (auto &RegionMatch : BestRegionMatch) {
    RegionInstructionAlignment RIAlignment(getInstrCostModel());
    RIAlignment.compute(RegionMatch->getMapping());
    auto InstrAlignment = RIAlignment.getResult();
    DCICompatibility.push_back(checkDCICompatible(InstrAlignment));
  }
}
...
void RegionAnalyzer::computeRegionMatch() {
  ...
  if (RunBranchFusionOnly) {
    ...
    if (LeftUniqueSucc && RightUniqueSucc &&
      ...
      BestBbMatchSimilarityScore =
          Utils::computeBlockSimilarity(LeftEntry, RightEntry, *InstrCostModel);
      DEBUG("Branch fusion can be applied to basic blocks " +
            LeftEntry->getNameAsOrOperand() + ", " +
            RightEntry->getNameAsOrOperand() + ", similarity score" +
            std::to_string(BestBbMatchSimilarityScore) + "\n");
  ...
  if (LeftRegions.empty() && RightRegions.empty()) {
    BestBbMatchSimilarityScore =
        Utils::computeBlockSimilarity(LeftEntry, RightEntry, *InstrCostModel);
    DEBUG("Basic blocks " + LeftEntry->getNameAsOrOperand() + ", " +
              RightEntry->getNameAsOrOperand() +
              " can be melded, similarity score "
          << std::to_string(BestBbMatchSimilarityScore) + "\n");
  ...
  if (LeftRegions.empty() || RightRegions.empty()) {
    ...
    if (BestBbMatch.first && BestBbMatch.second) {
      ...
      if (requireRegionReplication()) {
        ...
        BestBbMatchSimilarityScore =
            Utils::computeBlockSimilarity(BestBbMatch.first, BestBbMatch.second,
                                          ReplicatedRegion, *InstrCostModel);
        // BestBbMatch.first, BestBbMatch.second);

      } else {
        BestBbMatchSimilarityScore = Utils::computeBlockSimilarity(
            BestBbMatch.first, BestBbMatch.second, *InstrCostModel);
      }

      DEBUG("Similarity score = " + std::to_string(BestBbMatchSimilarityScore) +
            "\n");
    }
  ...
  for (unsigned I = 0; I < BestRegionMatch.size(); ++I) {
    ...
    ss << *BestPair;
    DEBUG(ss.str() + ", similarity score = " +
          std::to_string(BestPair->getSimilarityScore()) + "\n");
  }
}
...
RegionAnalyzer::findMostSimilarBb(BasicBlock *BB,
  ...
  double BestScore =
      Utils::computeBlockSimilarity(BB, MostSimilar, *InstrCostModel);
  // MergeableRegionPair::ComputeBBSimilarity(BB, candidates[0]);
  for (BasicBlock *Candidate : Candidates) {
    double Score =
        Utils::computeBlockSimilarity(BB, Candidate, *InstrCostModel);
    if (Score > BestScore) {
...
bool RegionAnalyzer::isRegionMatchCompatibleForDCI(unsigned Index) {
  assert(Index < regionMatchSize() && "Region match index out of bounds!");
  return DCICompatibility[Index];
}
