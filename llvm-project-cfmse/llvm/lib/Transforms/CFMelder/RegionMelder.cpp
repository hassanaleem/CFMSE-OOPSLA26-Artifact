    ...
    if (Entry.match()) {
      if (isa<BasicBlock>(LEntry)) {
        ...
        if (!isa<PHINode>(LEntry)) {
          ...
          // update merged debug info
          DebugInfoMerger::getInstance().updateMetaData(LeftI, RightI, NewI);
        }
      }
    } else {
...
void RegionMelder::setOperends(Instruction *LeftI, Instruction *RightI,
  ...
  for (unsigned I = 0; I < MergedI->getNumOperands(); I++) {
    ...
    if (LeftOp && RightOp && LeftOp != RightOp) {
      ...
      // set the debug location for the select
      if (MergedI->getDebugLoc()) {
        Select->setDebugLoc(MergedI->getDebugLoc());
        DebugInfoMerger::getInstance().updateMetaData(
            dyn_cast<Instruction>(LeftOp), dyn_cast<Instruction>(RightOp),
            Select);
      }
...
void RegionMelder::merge(unsigned Index) {
  ...
  if (MA.requireRegionReplication()) {
    DEBUG("Replicating regions in BB-region match\n");
    ...
    if (MA.requireRegionSimplification(RToReplicate)) {
      DEBUG("Replicated region is not a simple region, running region "
            "simplification\n");
      ...
      DEBUG("region after region simplification : " + "[" +
            RToReplicate->getEntry()->getNameOrAsOperand() + " : " +
            RToReplicate->getExit()->getNameOrAsOperand() + "]\n");
  ...
  DEBUG("Melding entry blocks " + EntryBlockL->getNameOrAsOperand() + " , " +
        EntryBlockR->getNameOrAsOperand() + "\n");

  if (ExitBlockL) {
    DEBUG("Melding exit blocks " + ExitBlockL->getNameOrAsOperand() + " , " +
          ExitBlockR->getNameOrAsOperand() + "\n");
  }
  ...
  if (!DisableMelding) {
...
#endif
    RegionInstructionAlignment RIA(MA.getInstrCostModel());
    RIA.compute(CurrMapping);
    RegionInstrAlignement = RIA.getResult();
#if ENABLE_TIMING == 1
    ...
    // insert dead code to make all instructions aligned properly
    if (InsertDeadCode) {

#ifdef REGIONMELDER_DEBUG
      static int Count = 0;
      writeCFGToDotFile(*MA.getParentFunction(),
                        "before_dc_insert_" + std::to_string(Count++) + ".");
#endif
      DeadCodeInserter DI(MA.getParentFunction(), RegionInstrAlignement,
                          MA.getCFGInfo().getDomTree());
      DI.run();
#ifdef REGIONMELDER_DEBUG
      writeCFGToDotFile(*MA.getParentFunction(),
                        "after_dc_insert_" + std::to_string(Count++) + ".");
#endif
      DEBUG("Verifying DCI...\n");
      for (auto &Entry : RegionInstrAlignement) {
        assert(Entry.match() && "mismatched instructions after DCI");
        Value *L = Entry.getLeft();
        Value *R = Entry.getRight();
        if (isa<Instruction>(L) && isa<Instruction>(R)) {
          assert(InstructionMatch::match(L, R) &&
                 "mismatched instructions after DCI");
        }
      }
      DEBUG("DCI successful!\n");
    }
...
void RegionMelder::runPostOptimizations() {
  ...
  DEBUG("Running post-merge optimizations\n");
      ...
      if (isa<BranchInst>(SinglePred->getTerminator())) {
        ...
        if (BI->isConditional() && (BI->getSuccessor(0) == SingleSucc ||
          ...
          for (auto &PHI : SingleSucc->phis()) {
            ...
            // set debug loc for select
            if (PHI.getDebugLoc()) {
              Sel->setDebugLoc(PHI.getDebugLoc());
            }
          }
        }
      }
