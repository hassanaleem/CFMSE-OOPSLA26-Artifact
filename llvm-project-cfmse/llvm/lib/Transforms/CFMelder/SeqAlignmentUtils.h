...
enum Direction { LEFT, TOP, DIAG, NONE };
...
public:
  Cell(int Value, Direction Direc, bool IsMatch)
      : Cost(Value), Direc(Direc), IsMatch(IsMatch) {}
...
template <typename elemTy> struct AlignedPair {
  elemTy EL = nullptr;
  elemTy ER = nullptr;
  bool Match = false;
...
public:
  ...
  void setLeft(elemTy EL) {
    this->EL = EL;
    this->Match = EL && ER;
  }
  void setRight(elemTy ER) {
    this->ER = ER;
    this->Match = EL && ER;
  }
...
public:
  ...
  // copy constructor
  AlignedSeq(const AlignedSeq &Other) {
    reserve(Other.size());
    insert(end(), Other.begin(), Other.end());
  }

  // copy assignment
  AlignedSeq &operator=(const AlignedSeq &Other) {
    reserve(Other.size());
    insert(end(), Other.begin(), Other.end());
    return *this;
  }
...
template <typename elemTy> class ScoringFunction {
public:
